...
Fund::Fund(double capitalization, int months_amount)
        : conventional_units(capitalization), capitalization(capitalization), delta_capitalization(0), deposit(months_amount + 1, 0),
          rng(std::time(nullptr)), names{"Egor", "Konstantin", "Damir", "Maksim", "Nikita", "Leonard", "Fedor", "Polin", "Arsen", "Mikhail"},
          surnames{"Osipov", "Yusupov", "Tursunov", "Alimov", "Saidov", "Alimov", "Yunusov", "Trifonov", "Kan", "Ivanov"} {}
...
double Fund::GetDeltaCapitalization() const {
    return delta_capitalization;
}
...
void Fund::Iterate(const Market &market, int month, double tax, double dividends) {
    ...
    if (delta_capitalization > 0) {
        int cntNewDepositors = delta_capitalization * 10 / capitalization * GetRandNum();
