...
Market::Market(double spread)
  ...
  // TODO: make this not a map but parts of smthn like class Currency
}
...
double Market::GetDepositPercent() const { return deposit_percent; }
...
double Market::GetSpread() const { return spread; }

void Market::Iterate() {
  for (int i = 1; static_cast<Currency>(i) != Currency::__Last; ++i) {
    currency_exchange_rate.at(static_cast<Currency>(i))
        .Iterate(GetRandNum(), spread);
  }
  deposit_percent *= 2 * (GetRandNum()) * spread / 10. + 1 - spread;
  deposit_percent = std::max(deposit_percent, 0.17);
}

double Market::GetRandNum() {
  return static_cast<double>(rng()) / static_cast<double>(rng.max());
}
const char *Market::DoesNotExistCurrency::what() const noexcept {
