...
System::System(int month_count, double capitalization, double tax,
               double spread)
    : month_cnt(1),
      month_amount(month_count),
      tax(tax),
      fund(capitalization, month_count),
      market(spread) {}

int System::GetMonthCount() const { return month_cnt; }
int System::GetMonthAmount() const { return month_amount; }
double System::GetCapitalization() const { return fund.GetCapitalization(); }
...
const char *System::UnreachableDeposit::what() const noexcept {
  return "Срок вклада превысил максимальный допустимый";
}
